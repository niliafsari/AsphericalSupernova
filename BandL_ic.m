...
% L_s=zeros(length(time_i(23:32)),1);
...
for i=1:length(bands)
    if (i==length(bands))
        ...
        %bandL(i,:,4)=((pi*L_s)./(sigma*Tp.^4)).*factor1;
...
subplot(3,1,1)
plot(time_ic(23:32)',log10(bandL(:,:,1)),'LineWidth',1.5)
axis([12 21 25 45])
str={'z^\prime', 'i^\prime','r^\prime', 'g^\prime', 'u^\prime', ...
   'NUV', 'FUV' ,'X', 'Bol'};

text(14,28,'Total Luminosity','HorizontalAlignment','left','FontSize',12);


set(gca,'LineWidth',1.5,'FontSize',12);
subplot(3,1,2)
plot(time_ic(23:32)',log10(bandL(:,:,2)),'LineWidth',1.5)
set(gca,'LineWidth',1.5,'FontSize',12);
ylabel('Log (L_{Band} [erg/sec])');
columnlegend(1,str,'location', 'northeast');
legend('boxoff')
text(14,30,'Luminosity \Phi=0','HorizontalAlignment','left','FontSize',12);
axis([12 21 25 45])
subplot(3,1,3)
plot(time_ic(23:32)',log10(bandL(:,:,3)),'LineWidth',1.5)
text(14,28,'Luminosity \Phi=\pi/2','HorizontalAlignment','left','FontSize',12);
axis([12 21 25 45])
samexaxis('abc','xmt','on','ytac','join','yld',1)
xlabel('t [sec]'); 
set(gca,'LineWidth',1.5,'FontSize',12);
name=['/home/nilou/Data/plot/BandL_ic.pdf'];    
print('-dpdf',name) 
export_fig(name, '-pdf')
