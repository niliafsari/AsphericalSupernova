...
% %  
    gaussKernel = 1*fspecial('disk', 7);
    dsmooth = imfilter(log10(real(tdiff/tdyn)),gaussKernel,'replicate'); 
...
%     
    ...
    h=surf(X/0.5,Y/0.5,log10(real(tdiff/tdyn)));hold on
    grid off
    ...
    xlabel('x/R_*');
    ylabel('y/R_*');
    title(['t/t_*=' num2str(time.time1(t)*tconv,3)]); 
    axis equal     
    
    C=contour3(X/0.5,Y/0.5,real(tdiff/tdyn),[1 1],'LineColor','k','Linewidth',1);
    C=contour3(X/0.5,Y/0.5,dsmooth,[0 0],'LineColor','r','Linewidth',1);
    set(gca,'LineWidth',2,'FontSize',12);
    caxis([-2 12]) 
    ylabel(cl,'$ Log \frac{t_{diff}}{t_{dyn}} $','interpreter','latex','FontSize',24);    
    view(2); 
    name=['/home/nilou/Data/plot/collision/colDiffFrontBSG_v2' num2str(t) '.png'];
