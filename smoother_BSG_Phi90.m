...
for t=23:41
    ...
    ydiff_n=diff_bsg(1,1:length(diff_bsg));
    xdiff_n=diff_bsg(2,1:length(diff_bsg)); 
    phidiff_n=atan(xdiff_n./ydiff_n);
    

    rdiff_n=rdiff_n(phidiff_n>=prctile(phidiff_n,1));
    xdiff_n=xdiff_n(phidiff_n>=prctile(phidiff_n,1));
    ydiff_n=ydiff_n(phidiff_n>=prctile(phidiff_n,1));
    phidiff_n=atan(xdiff_n./ydiff_n);
    ...
    axis equal
    axis([0 4 0 4])
    name=['/home/nilou/Data/plot/BSGdiff/BSG_filter_smoothed_Phi90' num2str(t) '.png'];
    print(gcf, '-dpng', '-r50', name)
    export_fig(name, '-dpng', '-r50')
end
save('/home/nilou/Data/processeddata/BSG/luminosity_smoothedk55_Phi90.mat','luminosity')
